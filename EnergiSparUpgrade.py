...
def calculate_savings(num_fixtures, hours_per_year, cost_per_kwh, tube_type, pris_armatur, additional_variable=0):
    ...
    saving_CO2 = (num_fixtures * 2.5) / 100  # 2.5 tons per 100 fixtures
...
additional_variable = st.number_input("Eventuell extra kostnad (t.ex. installation):", min_value=0, value=0)

if st.button("Beräkna besparingen"):
    old_cost, new_cost, savings, break_even, total_inpris, saving_CO2 = calculate_savings(
        num_fixtures, hours_per_year, cost_per_kwh, tube_type, pris_armatur, additional_variable
    )
    trees_required = calculate_trees(saving_CO2)
